...
public class User extends Thread{
	...
	// 임의로 프로그램 실행할 때 사용하기 위해서 선언한 객체
	...
	public void ioWork() { //처음 접속했을 때 한 번 실행되는 메소드
		try {
			...
			//checklist는 반복 잡업을 하지 않기 위해 만들어놓은 객체
			//callbyreference에 의해서 carlist와 checklist 변수가 carlist를 저장하는 hashmap을 공유
			...
			System.out.println("phone:"+userlist.size());
			System.out.println("car:"+carlist.size());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	//중복 확인 - 기존에 저장된 아이디가 중복 저장 되지 않도록 하기 위해서 체크. 기존 등록되어있는 사용자인지 확인
	...
	private void filteringMsg(String msg) {
		...
		if(protocol.equals("job")) {
			...
			System.out.println("message:"+message+"\tcategory:"+category+"\tid:"+id);
			// 서버에서 클라이언트의 메시지를 분석해서 메시지를 전달할 클라이언트를 정의
			User userclient = null;
			if(category.equals("phone")) {
				System.out.println("phone("+id+")에서 메시지를 보냄:");
				userclient = carlist.get(id); 
			}
			else {				
				System.out.println("car("+id+")에서 메시지를 보냄:");
